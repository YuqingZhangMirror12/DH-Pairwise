...
这是一份项目状态与研究方向快照，不是新训练已启动的通知，也不改变已有评估协议。主要训练结论截至 2026-09-20 20:48；补充了 09-21 的 dustbin 解码诊断。09-21 00:26 实查当前远端：7 张 RTX 4090 D 均空闲，无 GPU 计算进程。此前登记的七路实验与 42 个新分类头评估端点均已完成。
...
- 后续采用 S7 难化训练数据；新增主 Scorer 按 **4 层、96 维**设计，并对照 4／8 heads。注意：这不是当前新端点头已经训练的配置。
...
- Pair：`logit`、`sigmoid(logit)`，与仿真验证集冻结阈值比较；还须保留既有 `decision_valid` 语义。
...
1. **先把局部证据主线固定。** 以 S7 M12＋matched_tokens 为控制，主新架构按用户要求 4 层96D，保留同预算基线；4/8 heads 单独对照。全量 token 仅保留控制，不删除旧代码和结果。
2. **分开“这一对能拼”与“这个候选是真的”。** 保留 Pair 邻接监督，另行设计候选正确性／排序监督。正 pair 的错误候选不能全部当正局部证据；也不能反过来把整个 pair 改成负标签。推理不得读取 GT。
3. **让训练输出覆盖真实局部证据。** 不只增加腐蚀名称或样本总量，重点补“少支持但可信、高残差但可拼”的正例，与几何看似一致的困难负例；在独立困难仿真验证上选模型。
4. **对应强度与 dustbin 分开做增量。** 同一局部特征／候选上，单独对照 Q、两侧未匹配信息、残差表达；不要一次同时改数据、Matcher、解码和分类头后无法解释收益。
5. **候选稳定性和物理尺度。** 多候选应保留相互竞争关系，不简单把多个位置的点混成一条缝；可以研究软选择、间距一致的 context、局部缺口容忍与 overlap，但不能标记为已验证修复。

判断是否进步应同时看：Pair 排序／误报－召回、原始 Layout、正确 Layout 通过分类比例、候选覆盖／排序，以及少支持、部分接缝、腐蚀子组。不得用 REAL 选择新阈值，再在同一批 REAL 上当作外部成绩。
...
不要写：全轮廓特征完全无用；Cross-Attention 已证明唯一正确；Q/dustbin 一加就好；有 Layout 候选必能拼；四层已是本轮头；训练轮数已充分到不需再验证；人工筛选后成绩代表无偏真实泛化；Turufan 有可验证的二分类 F1 或 Layout 正确率。
