"""E2: identical E1 data + clean/corrupt consistency and source-corrected gap loss.
...
TRAIN_COUNT = 24_000
VAL_COUNT = 3_000
SCHEMA = "rachel-edge-consistency-training/1"
...
def _weathering_interfaces():
    ...
    from staging.pairwise_v0_2.training.rachel_weathering_consistency import (
        E2Dataset, make_e2_loader, train_e2_epoch,
    )
    return E2Dataset, make_e2_loader, train_e2_epoch
...
def resume_identity(source_path, manifest_path, original_root, architecture):
    ...
    from staging.pairwise_v0_2.training.rachel_weathering_consistency import E2LossConfig
    source_sha = _sha256(source_path)
    if source_sha != SOURCE_SHA256:
        raise ValueError("E2 requires the original frozen 350bf9 warm-start checkpoint")
    return dict(schema_version=SCHEMA, source_checkpoint=str(source_path),
                ...
                e2_loss_config=asdict(E2LossConfig()))
...
def checkpoint_payload(model, loss_config, source, source_path, data_record, protocol,
    ...
    return dict(model_state_dict=_cpu_model_state(model), **model_metadata(model),
                ...
                validation_event=validation_events, variant="edge_consistency_e2", initialization="warm-start",
                ...
                e2_loss_config=protocol["weathering_policy"]["e2_loss_config"])
...
def run(args):
    ...
    if not torch.cuda.is_available() or not args.device.startswith("cuda"):
        raise RuntimeError("E2 full-network training must run explicitly on the remote CUDA server")
    ...
    if len(base_training) != TRAIN_COUNT or data_record["unique_count"] != TRAIN_COUNT or len(validation) != VAL_COUNT:
        raise ValueError("E2 requires unique matched TRAIN24k and unchanged clean VAL3000")
    data_record = dict(data_record, target_policy="E1 source-arc correspondence plus E2 consistency and source-corrected gap target")
    ...
    from staging.pairwise_v0_2.training.rachel_weathering_consistency import E2LossConfig
    policy = dict(e2_loss_config=asdict(E2LossConfig()),
                  e1_data_and_source_arc_inheritance_unchanged=True,
                  consistency_teacher="online same model, clean eval/no_grad; no EMA or additional trainable parameters",
                  consistency_policy="changed-pair three-branch probability MSE and source-aligned bidirectional partial-assignment KL",
                  gap_policy="inherited seam anchor rows/columns; assignment-weighted source-corrected displacement error, allowing known material-loss gap",
                  comparison_budget="same 120k pair draws and 7500 updates as E1; additional clean teacher forward is extra compute, not a second optimizer exposure",
                  requested_tier_grain="fragment endpoint exposure, not pair", requested_endpoint_tier_probabilities=dict(clean=.70, mild=.25, moderate=.05),
    ...
    protocol = dict(schema_version=SCHEMA, status="running", arm="edge_consistency_e2", initialization="warm-start",
    ...
    try:
        for segment_index, segment in enumerate(plan, 1):
            ...
            train_report = train_epoch(model, loader, optimizer, loss_config, device, args, epoch, "edge_consistency_e2")
        ...
        if (total_samples, total_updates, validation_events) != (TOTAL_EXPOSURES, TOTAL_EXPOSURES // EFFECTIVE_BATCH, TOTAL_EXPOSURES // VALIDATION_INTERVAL):
            raise RuntimeError("completed E2 run did not meet 120k exposures/7500 updates/5 clean VAL events")
        if best_key is None:
...
def parser():
    ...
    p.add_argument("--output", required=True, help="new E2 training directory (or existing with --resume)")
